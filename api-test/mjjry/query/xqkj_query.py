...
from mjjry.testAction import loginAction as la
...
def update_contract_machine_pass(contract_uuid, user_uuid):
    ...
    contract_name = {'服务完结确认书_QA.pdf': "A", '应收账款转让协议书_QA.pdf': "B", '债权转让通知书_QA.pdf': "C",
                     '居间服务协议_QA.pdf': "D", '医疗美容项目服务合同_QA.pdf': "E"}
    ...
    for _name in contract_name.keys():
        ...
        if contract_info_rows == 0:
            insert_dict = {'contract_info_uuid': str(uuid.uuid1()).split('-'), 'contract_uuid': contract_uuid,
                           'fdd_number': "2020011618014346820480_" + contract_name[_name], 'contract_name': _name,
                           'type': '1', 'platform': '1', 'sign_state': '0', 'view_url': '', 'download_url': '',
                          'state': 'enabled'}

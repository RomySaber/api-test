...
def yys_modif(yys_data, modif_key, modif_value):
    ...
    try:
        if modif_key == 'V03_1002':
            # 开户时长(月)
            m_value = TimeFormat.get_day_around(FileUtils.str_to_num(str(modif_value)))
...
def jingdong_modif(jd_data, modif_key, modif_value):
    ...
    """
    try:
        if modif_key == 'V04_1002':
            jd_data["credit"] = modif_value
        elif modif_key == 'V04_1005':
            jd_data["BtPrivilege"]["creditLimit"] = modif_value
        elif modif_key == 'V04_1008':
            jd_data["creditData"]["jtDelinquencyBalance"] = modif_value
        else:
            raise KeyError('the modif_key {} is error'.format(modif_key))
    except Exception as e:
        LOGGER.error(e)
...
def taobao_modif(tb_data, modif_key, modif_value):
    ...
    """
    try:
        if modif_key == 'V05_1009':
            tb_data["alipayInfo"]["huabei_credit"] = modif_value
        elif modif_key == 'V05_1010':
            tb_data["alipayInfo"]["huabei_available"] = modif_value
        else:
            raise KeyError('the modif_key {} is error'.format(modif_key))
    except Exception as e:
        LOGGER.error(e)

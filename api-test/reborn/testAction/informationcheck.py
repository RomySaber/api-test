...
API_TEST_HEADERS = loginadmin.streamadminlogin()
#
# API_TEST_HEADERS = globaltoken.get_value('name','score')
# print('登录的管理员token',API_TEST_HEADERS)
token=API_TEST_HEADERS['mytoken']
...
###############################################################################
    ...
    def test_api_78dk_platform_cm_base_business_saveBusinessInfor(self, businessaddress, businessaddressgpsloction, businessaddresszipcode, businesshoursendtime, businesshoursstarttime, businessinformationuuid, businessregistrationnumber, channelormerchantuuid, documentaddress, email, organizationcode, socialunifiedcreditcode, storerentalendtime, storerentalstarttime, taxregistrationnumber):
        """
        添加机构
...
###############################################################################
    ...
    def test_api_78dk_platform_cm_base_saveChannel(self, city, name, parentchanneluuid, province, region, shortname):
        ...
        params = json.dumps({"city": city, "name": name, "parentChannelUuid": parentchanneluuid, "province": province, "region": region, "shortName": shortname})
    ...
    def test_api_78dk_platform_cm_base_updateChannel(self, channeluuid, city, name, note, operatoruuid, province, region, shortname):
        """
        编辑渠道
        :return: response.text
        """
        ...
        params = json.dumps({"channelUuid": channeluuid, "city": city, "name": name, "note": note, "operatorUuid": operatoruuid, "province": province, "region": region, "shortName": shortname})
    ...
    def test_api_78dk_platform_cm_state_viewStateChannels(self, name, pagecurrent, pagesize,openCloseState,freezeState,auditState):
        """
        渠道状态列表
        :return: response.text
        """
        ...
        params = json.dumps({"name": name, "pageCurrent": pagecurrent, "pageSize": pagesize,"openCloseState":openCloseState,"freezeState":freezeState,"auditState":auditState})
...
###############################################################################
    ...
    def test_api_78dk_platform_cm_base_operator_saveOperator(self, channelormerchantuuid, mail, mobile, name, password, salt, title):
        """
        添加操作员
        :return: response.text
        """
        ...
        params = json.dumps({"channelOrMerchantUuid": channelormerchantuuid, "mail": mail, "mobile": mobile, "name": name, "password": password, "salt": salt, "title": title})
    ...
    def test_api_78dk_platform_cm_base_operator_updateOperator(self, channelormerchantuuid, mail, mobile, name, operatoruuid, password, salt, title):
        """
        编辑操作员
        :return: response.text
        """
        ...
        params = json.dumps({"channelOrMerchantUuid": channelormerchantuuid, "mail": mail, "mobile": mobile, "name": name, "operatorUuid": operatoruuid, "password": password, "salt": salt, "title": title})
...
###############################################################################
    ...
    def test_api_78dk_platform_cm_base_legal_saveLegalPerson(self, cardnumber, channelormerchantuuid, legalpersonuuid, mobile, name):
        """
        添加法人代表
        :return: response.text
        """
        ...
        params = json.dumps({"cardNumber": cardnumber, "channelOrMerchantUuid": channelormerchantuuid, "legalPersonUuid": legalpersonuuid, "mobile": mobile, "name": name})
    ...
    def test_api_78dk_platform_cm_base_legal_updateLegalPerson(self, cardnumber, channelormerchantuuid, legalpersonuuid, mobile, name):
        """
        编辑法人代表
        :return: response.text
        """
...
###############################################################################
    ...
    def test_api_78dk_platform_cm_base_clear_saveClearingAccount(self, accountname, accountnumber, accountopeningbank, accounttype, branchname, chamberlainidcardnumber, channelormerchantuuid, city, clearingaccountuuid, linenumber, phone, province, region):
        """
        添加渠道结算信息
        :return: response.text
        """
        ...
        params = json.dumps({"accountName": accountname, "accountNumber": accountnumber, "accountOpeningBank": accountopeningbank, "accountType": accounttype, "branchName": branchname, "chamberlainIdcardNumber": chamberlainidcardnumber, "channelOrMerchantUuid": channelormerchantuuid, "city": city, "clearingAccountUuid": clearingaccountuuid, "lineNumber": linenumber, "phone": phone, "province": province, "region": region})
    ...
    def test_api_78dk_platform_cm_base_clear_updateClearingAccount(self, accountname, accountnumber, accountopeningbank, accounttype, branchname, chamberlainidcardnumber, channelormerchantuuid, city, clearingaccountuuid, linenumber, phone, province, region):
        """
        编辑渠道结算信息
        :return: response.text
        """
...
###############################################################################
    ...
    def test_api_78dk_platform_mm_base_saveMerchant(self, channeluuid, city, name, note, org, parentmerchantuuid, province, region, shortname):
        """
        新增商户基本信息
        :return: response.text
        """
        ...
        params = json.dumps({"channelUuid": channeluuid, "city": city, "name": name, "note": note, "org": org, "parentMerchantUuid": parentmerchantuuid, "province": province, "region": region, "shortName": shortname})
    ...
    def test_api_78dk_platform_mm_base_updateMerchant(self, channeluuid, city, merchantuuid, name, note, org, parentmerchantuuid, province, region, shortname):
        """
        修改商户基本信息
        :return: response.text
        """
        ...
        params = json.dumps({"channelUuid": channeluuid, "city": city, "merchantUuid": merchantuuid, "name": name, "note": note, "org": org, "parentMerchantUuid": parentmerchantuuid, "province": province, "region": region, "shortName": shortname})
...
###############################################################################
    ...
    def test_api_78dk_platform_mm_base_business_saveBusinessInfor(self, businessaddress, businessaddressgpsloction, businessaddresszipcode, businesshoursendtime,
                                                                  businesshoursstarttime, businessinformationuuid, businessregistrationnumber,
                                                                  channelormerchantuuid, documentaddress, email, organizationcode, socialunifiedcreditcode,
                                                                  storerentalendtime, storerentalstarttime, taxregistrationnumber,businessProvince,BusinessCity,BusinessRegion,DocumentProvince,DocumentCity,DocumentRegion):
        """
        新增机构信息
        :return: response.text
        """
        ...
        params = json.dumps({"businessAddress": businessaddress, "businessAddressGpsLoction": businessaddressgpsloction, "businessAddressZipCode": businessaddresszipcode, "businessHoursEndTime": businesshoursendtime, "businessHoursStartTime": businesshoursstarttime, "businessInformationUuid": businessinformationuuid,
                             "businessRegistrationNumber": businessregistrationnumber, "channelOrMerchantUuid": channelormerchantuuid, "documentAddress": documentaddress, "email": email, "organizationCode": organizationcode, "socialUnifiedCreditCode": socialunifiedcreditcode, "storeRentalEndTime": storerentalendtime, "storeRentalStartTime": storerentalstarttime,
                             "taxRegistrationNumber": taxregistrationnumber,"businessProvince":businessProvince,"BusinessCity":BusinessCity,"BusinessRegion":BusinessRegion,"DocumentProvince":DocumentProvince,"DocumentCity":DocumentCity,"DocumentRegion":DocumentRegion})
    ...
    def test_api_i78dk_platform_mm_base_business_updateBusinessInfor(self, businessaddress, businessaddressgpsloction, businessaddresszipcode, businesshoursendtime, businesshoursstarttime, businessinformationuuid, businessregistrationnumber, channelormerchantuuid, documentaddress, email, organizationcode, socialunifiedcreditcode, storerentalendtime, storerentalstarttime,
                                                                     taxregistrationnumber,businessProvince,BusinessCity,BusinessRegion,DocumentProvince,DocumentCity,DocumentRegion):
        """
        修改机构信息
        ...
        params = json.dumps({"businessAddress": businessaddress, "businessAddressGpsLoction": businessaddressgpsloction, "businessAddressZipCode": businessaddresszipcode, "businessHoursEndTime": businesshoursendtime, "businessHoursStartTime": businesshoursstarttime, "businessInformationUuid": businessinformationuuid, "businessRegistrationNumber": businessregistrationnumber, "channelOrMerchantUuid": channelormerchantuuid, "documentAddress": documentaddress, "email": email, "organizationCode": organizationcode, "socialUnifiedCreditCode": socialunifiedcreditcode, "storeRentalEndTime": storerentalendtime, "storeRentalStartTime": storerentalstarttime, "taxRegistrationNumber": taxregistrationnumber,"businessProvince":businessProvince,"BusinessCity":BusinessCity,"BusinessRegion":BusinessRegion,"DocumentProvince":DocumentProvince,"DocumentCity":DocumentCity,"DocumentRegion":DocumentRegion})
    ...
    def test_api_78dk_platform_mm_base_business_viewBusinessInforByMerchant(self, uid):
        ...
        self.LOGGER.info("test_api_78dk_platform_mm_base_business_viewBusinessInforByMerchant请求地址:【{}】".format(requesturl))
        params = json.dumps({"uid": uid})
...
###############################################################################
    ...
    def test_api_78dk_platform_mm_base_legal_saveLegalPerson(self, cardnumber, channelormerchantuuid, legalpersonuuid, mobile, name):
        """
        新增商户法人信息
        :return: response.text
        """
    ...
    def test_api_78dk_platform_mm_base_legal_updateLegalPerson(self, cardnumber, channelormerchantuuid, legalpersonuuid, mobile, name):
        """
        修改商户法人信息
        :return: response.text
        """
...
###############################################################################
    ...
    def test_api_78dk_platform_mm_base_clear_saveClearingAccount(self, accountname, accountnumber, accountopeningbank, accounttype, branchname, chamberlainidcardnumber, channelormerchantuuid, city, clearingaccountuuid, linenumber, phone, province, region):
        """
        新增商户结算信息
        :return: response.text
        """
    ...
    def test_api_78dk_platform_mm_base_clear_updateClearingAccount(self, accountname, accountnumber, accountopeningbank, accounttype, branchname, chamberlainidcardnumber, channelormerchantuuid, city, clearingaccountuuid, linenumber, phone, province, region):
        """
        修改商户结算信息
        :return: response.text
        """
    ...
    def test_api_78dk_platform_mm_base_clear_viewClearingAccountByMerchant(self, uid):
        ...
        self.LOGGER.info("test_api_78dk_platform_mm_base_clear_viewClearingAccountByMerchant请求地址:【{}】".format(requesturl))
        params = json.dumps({"uid": uid})
...
###############################################################################
    ...
    def test_api_78dk_platform_mm_base_store_saveStore(self, businessaddress, businessaddressgpsloction, managername, managerphone, merchantuuid,
        ...
        params = json.dumps({"businessAddress": businessaddress, "businessAddressGpsLoction": businessaddressgpsloction, "managerName": managername,
                             "managerPhone": managerphone, "merchantUuid": merchantuuid, "stormUuid": stormuuid,"Province":Province,"City":City,"region":region,"storeName":storeName})
    ...
    def test_api_78dk_platform_mm_base_store_updateStore(self, businessaddress, businessaddressgpsloction, managername, managerphone, merchantuuid, stormuuid,Province,City,region,storeName):
        """
        修改商户门店信息
        :return: response.text
        """
        ...
        params = json.dumps({"businessAddress": businessaddress, "businessAddressGpsLoction": businessaddressgpsloction, "managerName": managername, "managerPhone": managerphone, "merchantUuid": merchantuuid, "stormUuid": stormuuid,"Province":Province,"City":City,"region":region,"storeName":storeName})
...
###############################################################################
    ...
    def test_api_78dk_platform_mm_money_saveMerchantMoney(self, amountday, amountmonth, amountsingle, amountsum, merchantuuid, moneyconfiguuid, zoomcoefficient):
        """
        新增额度管理
        :return: response.text
        """
        ...
        params = json.dumps({"amountDay": amountday, "amountMonth": amountmonth, "amountSingle": amountsingle, "amountSum": amountsum, "merchantUuid": merchantuuid, "moneyConfigUuid": moneyconfiguuid, "zoomCoefficient": zoomcoefficient})
    ...
    def test_api_78dk_platform_mm_money_updateMerchantMoney(self, amountday, amountmonth, amountsingle, amountsum, merchantuuid, moneyconfiguuid, zoomcoefficient):
        """
        修改额度管理
        :return: response.text
        """
...
###############################################################################
    ...
    def test_api_78dk_platform_sys_user_updateSystemUser(self, email, mobile, name, platformuserprofileuuid):
        ...
        params = json.dumps({"email": email, "mobile": mobile, "name": name, "platformUserProfileUuid": platformuserprofileuuid})
...
###############################################################################
    ...
    def test_api_78dk_platform_sys_privilege_saveUserPrivilege(self, platformprivilegeuuid, platformuseruuid):
        ...
        params = json.dumps([{"platformPrivilegeUuid": platformprivilegeuuid, "platformUserUuid": platformuseruuid},
                             {"platformPrivilegeUuid":"5db0d9fb5d524a098810855385fb042e","platformUserUuid":platformuseruuid},
                             {"platformPrivilegeUuid":"15b0d9fb5d524a098810855385fb042e","platformUserUuid":platformuseruuid},
                             {"platformPrivilegeUuid": "685424dff6b24278a77281f2367be331", "platformUserUuid": platformuseruuid},
                             {"platformPrivilegeUuid": "67b0d9fb5d524a098810855385fb042e","platformUserUuid": platformuseruuid}])
    ...
    def test_api_78dk_platform_sys_privilege_saveUserPrivilegesuperadmin(self, platformprivilegeuuid, platformuseruuid):
        ...
        params = json.dumps([{"platformPrivilegeUuid": platformprivilegeuuid, "platformUserUuid": platformuseruuid},
                             {"platformPrivilegeUuid":"5db0d9fb5d524a098810855385fb042e","platformUserUuid":platformuseruuid},
                             {"platformPrivilegeUuid":"15b0d9fb5d524a098810855385fb042e","platformUserUuid":platformuseruuid},
                             {"platformPrivilegeUuid": "685424dff6b24278a77281f2367be331", "platformUserUuid": platformuseruuid},  #决策中心
                             {"platformPrivilegeUuid": "67b0d9fb5d524a098810855385fb042e","platformUserUuid": platformuseruuid}])
        self.LOGGER.info("test_api_78dk_platform_sys_privilege_saveUserPrivilege请求参数：【{}】".format(params))
        response = requests.request(
                "POST", requesturl, headers=API_TEST_HEADERS, data=params
...
###############################################################################
    ...
    def test_api_78dk_platform_fund_fundPackage_saveFundPackage(self, amount, contracttemplate, fundsideuuid, name, state):
        """
        添加资金包
        :return: response.text
        """
        ...
        params = json.dumps({"amount": amount, "contractTemplate": contracttemplate, "fundSideUuid": fundsideuuid, "name": name, "state": state})
    ...
    def test_api_78dk_platform_fund_fundPackage_updateFundPackage(self, amount, contracttemplate, fundpackageuuid, fundsideuuid, name, state):
        """
        编辑资金包
        :return: response.text
        """
        ...
        params = json.dumps({"amount": amount, "contractTemplate": contracttemplate, "fundPackageUuid": fundpackageuuid, "fundSideUuid": fundsideuuid, "name": name, "state": state})
...
###############################################################################
    ...
    def test_api_78dk_platform_product_base_saveProduct(self, discountrate, earlyrepaymentfreecycle, earlyrepaymenthandlingfee, earlyrepaymentsupport, firsthalfofthemonth, fundpackageuuid,
                                                        maxquota, minquota, name, overduegraceperiod, overduehandlingfeerate, overdueprincipalrate, productconfigs, productstate, remark, repaymentdatetype, repaymentmethod, secondhalfofthemonth, state):
        """
        添加产品模板
        :return: response.text
        """
        ...
        params = json.dumps({"discountRate": discountrate, "earlyRepaymentFreeCycle": earlyrepaymentfreecycle, "earlyRepaymentHandlingFee": earlyrepaymenthandlingfee, "earlyRepaymentSupport": earlyrepaymentsupport, "firstHalfOfTheMonth": firsthalfofthemonth, "fundPackageUuid": fundpackageuuid, "maxQuota": maxquota, "minQuota": minquota, "name": name, "overdueGracePeriod": overduegraceperiod, "overdueHandlingFeeRate": overduehandlingfeerate, "overduePrincipalRate": overdueprincipalrate, "productConfigs": productconfigs, "productState": productstate, "remark": remark, "repaymentDateType": repaymentdatetype, "repaymentMethod": repaymentmethod, "secondHalfOfTheMonth": secondhalfofthemonth, "state": state})
    ...
    def test_api_78dk_platform_product_base_updateProduct(self, discountrate, earlyrepaymentfreecycle, earlyrepaymenthandlingfee, earlyrepaymentsupport, firsthalfofthemonth, fundpackageuuid, maxquota, minquota, name, overduegraceperiod, overduehandlingfeerate, overdueprincipalrate, productconfigs, productdetailuuid, productstate, remark, repaymentdatetype, repaymentmethod, secondhalfofthemonth, state):
        ...
        params = json.dumps({"discountRate": discountrate, "earlyRepaymentFreeCycle": earlyrepaymentfreecycle, "earlyRepaymentHandlingFee": earlyrepaymenthandlingfee, "earlyRepaymentSupport": earlyrepaymentsupport, "firstHalfOfTheMonth": firsthalfofthemonth, "fundPackageUuid": fundpackageuuid, "maxQuota": maxquota, "minQuota": minquota, "name": name, "overdueGracePeriod": overduegraceperiod, "overdueHandlingFeeRate": overduehandlingfeerate, "overduePrincipalRate": overdueprincipalrate, "productConfigs": productconfigs, "productDetailUuid": productdetailuuid, "productState": productstate, "remark": remark, "repaymentDateType": repaymentdatetype, "repaymentMethod": repaymentmethod, "secondHalfOfTheMonth": secondhalfofthemonth, "state": state})
...
###############################################################################
    ...
    def test_api_78dk_platform_product_pmm_saveMerchantTX(self, discountrate, merchantuuid, period, productdetailconfiguuid, rate, discountrate2, merchantuuid2, period2, productdetailconfiguuid2, rate2):
        """
        ...
        params = json.dumps([{"discountRate": discountrate, "merchantUuid": merchantuuid, "period": period, "productDetailConfigUuid": productdetailconfiguuid, "rate": rate},{"discountRate": discountrate2, "merchantUuid": merchantuuid2, "period": period2, "productDetailConfigUuid": productdetailconfiguuid2, "rate": rate2}])
...
###############################################################################
    ...
    def test_api_78dk_platform_tm_telephone_addTelephoneCheckInfos(self, contract_uuid, telephoneCheckFeedbackUuid1,
                                                                   telephoneCheckFeedbackUuid2,
                                                                   telephoneCheckFeedbackUuid3,
                                                                   telephoneCheckFeedbackUuid4,
                                                                   telephoneCheckFeedbackUuid5,
                                                                   telephoneCheckFeedbackUuid6,
                                                                   telephoneCheckFeedbackUuid7,
                                                                   telephoneCheckFeedbackUuid8,
                                                                   telephoneCheckFeedbackUuid9,
                                                                   telephoneCheckFeedbackUuid10):
        """
        批量添加电核资料
        依赖函数：test_{'data': '返回数据', 'code': '返回码', 'msg': '返回消息文本'}
        依赖参数：
        :return: response.text
        """
        par = [{"answer": "rrrr", "contractUuid": contract_uuid, "groupName": "客户信息", "groupSort": 1, "question": "名字",
                "questionSort": 1, "replyType": "reply_type_error", "riskType": "risk_type_optional",
                "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid1},
               {"answer": "", "contractUuid": contract_uuid, "groupName": "客户信息", "groupSort": 1, "question": "证件号后6位",
                "questionSort": 2, "replyType": "reply_type_error", "riskType": "risk_type_optional",
                "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid2},
               {"answer": "", "contractUuid": contract_uuid, "groupName": "客户信息", "groupSort": 1, "question": "年龄",
                "questionSort": 3, "replyType": "reply_type_error", "riskType": "risk_type_optional",
                "telephoneCheckFeedbackUuid":telephoneCheckFeedbackUuid3},
               {"answer": "yuytutyutyuyt", "contractUuid": contract_uuid, "groupName": "客户信息", "groupSort": 1,
                "question": "属相", "questionSort": 4, "replyType": "reply_type_correct",
                "riskType": "risk_type_optional", "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid4},
               {"answer": "vvvvvv", "contractUuid": contract_uuid, "groupName": "客户信息", "groupSort": 1,
                "question": "婚姻状况", "questionSort": 5, "replyType": "reply_type_error",
                "riskType": "risk_type_optional", "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid5},
               {"answer": "rrr", "contractUuid": contract_uuid, "groupName": "客户信息", "groupSort": 1, "question": "职业类型",
                "questionSort": 6, "replyType": "reply_type_error", "riskType": "risk_type_optional",
                "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid6},
               {"answer": "rrrr", "contractUuid": contract_uuid, "groupName": "客户信息", "groupSort": 1,
                "question": "装修地址", "questionSort": 7, "replyType": "reply_type_correct",
                "riskType": "risk_type_optional", "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid7},
               {"answer": "rrr", "contractUuid": contract_uuid, "groupName": "合同信息", "groupSort": 2, "question": "合同金额",
                "questionSort": 8, "replyType": "reply_type_correct", "riskType": "risk_type_optional",
                "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid8},
               {"answer": "rrrrrrrrrr", "contractUuid": contract_uuid, "groupName": "合同信息", "groupSort": 2,
                "question": "本次贷款金额", "questionSort": 9, "replyType": "reply_type_correct",
                "riskType": "risk_type_optional", "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid9},
               {"answer": "rrrrrrrrrrrrrrrrrr", "contractUuid": contract_uuid, "groupName": "合同信息", "groupSort": 2,
                "question": "贷款期限", "questionSort": 10, "replyType": "reply_type_error",
                "riskType": "risk_type_optional", "telephoneCheckFeedbackUuid": telephoneCheckFeedbackUuid10},
               {"answer": "", "contractUuid": contract_uuid, "groupName": "其他信息", "groupSort": "3",
                "question": "咕咕咕咕过过过过过过过过", "questionSort": "1", "replyType": "reply_type_correct",
                "riskType": "risk_type_optional", "telephoneCheckFeedbackUuid": ""},
               {"answer": "", "contractUuid": contract_uuid, "groupName": "其他信息", "groupSort": "3",
                "question": " 反反复复付付付付付付付付", "questionSort": "1", "replyType": "reply_type_correct",
                "riskType": "risk_type_optional", "telephoneCheckFeedbackUuid": ""},
               {"answer": "发个梵蒂冈梵蒂冈梵蒂冈发的", "contractUuid": contract_uuid, "groupName": "其他信息",
                "groupSort": "3", "question": "个的发个梵蒂冈梵蒂冈梵蒂冈", "questionSort": "1", "replyType": "reply_type_correct",
                "riskType": "risk_type_optional", "telephoneCheckFeedbackUuid": ""}]

        requesturl = baseUrl + "/api/78dk/platform/tm/telephone/addTelephoneCheckInfos"
        self.LOGGER.info("test_api_78dk_platform_tm_telephone_addTelephoneCheckInfos请求地址:【{}】".format(requesturl))
        params = json.dumps(par)
        self.LOGGER.info("test_api_78dk_platform_tm_telephone_addTelephoneCheckInfos请求参数：【{}】".format(params))
        response = requests.request(
            "POST", requesturl, headers=API_TEST_HEADERS, data=params
        )
        self.LOGGER.info("请求结果参数：【{}】".format(response.text))
    ...
    def test_api_78dk_platform_tm_telephone_viewTelephoneCheckInfosByContractUuid(self, uid,):
        ...
        self.LOGGER.info("test_api_78dk_platform_tm_telephone_viewTelephoneCheckInfosByContractUuid请求地址:【{}】".format(requesturl))
        params = json.dumps({"uid": uid, })
        self.LOGGER.info("test_api_78dk_platform_tm_telephone_viewTelephoneCheckInfosByContractUuid请求参数：【{}】".format(params))
        response = requests.request(
                "POST", requesturl, headers=API_TEST_HEADERS, data=params
        )
        self.LOGGER.info("请求结果参数：【{}】".format(response.text))
    ...
    def test_api_78dk_platform_tm_first_saveSupplementImage(self, backgroundsupplementimages, contractuuid, supplementimagetype,):
        """
        提交或编辑补录资料
        ...
        params = json.dumps({"backGroundSupplementImages": backgroundsupplementimages, "contractUuid": contractuuid, "supplementImageType": supplementimagetype, })
    ...
    def test_api_78dk_platform_tm_first_passContract(self, audituuid, contractuuid, description, supplementimagerequires,):
        """
        打回初审的合同
        依赖函数：test_{'data': '返回数据', 'code': '返回码', 'msg': '返回消息文本'}
        依赖参数：
        ...
        params = json.dumps({"auditUuid": audituuid, "contractUuid": contractuuid, "description": description, "supplementImageRequires": supplementimagerequires, })
    ...
    def test_api_78dk_platform_tm_first_addAuditCommentfujian(self, contractuuid,):
        """
        添加一条评论
        依赖函数：test_{'data': "{'comment': '', 'auditCommentUuid': '评论UUID', 'created': '', 'auditUser': '审核人员', 'contractUuid': '', 'state': '', 'auditUserUuid': '', 'updated': '', 'replyAuditCommentUuid': ''}", 'code': '返回码', 'msg': '返回消息文本'}
        依赖参数：
        ...
        self.LOGGER.info("test_api_78dk_platform_tm_first_addAuditComment请求地址:【{}】".format(requesturl))
        # params = json.dumps({"auditCommentAttachments": auditcommentattachments, "comment": comment, "contractUuid": contractuuid, "replyAuditCommentUuid": replyauditcommentuuid, })
        params = json.dumps( {"comment":"<p>接口添加附件3333333333333333333333333333333</p><p></p><p></p><div class=\"media-wrap image-wrap\"><img src=\"http://test.jtlbasicupload.78dk.com/images//2db6/99882db693a9aabc1f7a54ce7a5e1b4a805a.png?MyToken={}&amp;dataType=plat\"/></div><p></p>".format(token),
"contractUuid":contractuuid,
"auditCommentAttachments":[{"attachmentName":"1542959904(1).png","attachmentSize":9.75390625,"attachmentType":"image","url":"2db6/99882db693a9aabc1f7a54ce7a5e1b4a805a.png"}]})
        self.LOGGER.info("test_api_78dk_platform_tm_first_addAuditComment请求参数：【{}】".format(params))
        response = requests.request(
                "POST", requesturl, headers=API_TEST_HEADERS, data=params
        )
    ...
    def test_api_78dk_platform_tm_telephone_saveTelephoneCheckInfos(self, answer, contractuuid, groupname, groupsort, question, questionsort, replytype,
        ...
        params = json.dumps({"answer(N)": answer, "contractUuid(Y)": contractuuid, "groupName(Y)": groupname, "groupSort(Y)": groupsort, "question(Y)": question,
    ...
    def test_api_78dk_platform_tm_first_updateContractInfoSignState(self, uid,):
        """
        修改合同状态为重签
        依赖函数：test_{'code': '返回码', 'data': '返回数据', 'msg': '返回消息文本'}
        依赖参数：
        :return: response.text
        """
        requesturl = baseUrl + "/api/78dk/platform/tm/first/updateContractInfoSignState"
        self.LOGGER.info("test_api_78dk_platform_tm_first_updateContractInfoSignState请求地址:【{}】".format(requesturl))
        params = json.dumps({"uid": uid, })
        self.LOGGER.info("test_api_78dk_platform_tm_first_updateContractInfoSignState请求参数：【{}】".format(params))
        response = requests.request(
                "POST", requesturl, headers=API_TEST_HEADERS, data=params
        )
        self.LOGGER.info("请求结果参数：【{}】".format(response.text))
    ...
    def test_api_78dk_platform_tm_first_findContractInfoSignStateWeb(self, uid,):
        """
        修改法大大合同签署状态 修改为重签
        依赖函数：test_{'code': '返回码', 'data': "{'signStateName': '合同类型', 'contractUuid': '合同uuid', 'signState': '合同类型'}", 'msg': '返回消息文本'}
        依赖参数：
        :return: response.text
        """
        requesturl = baseUrl + "/api/78dk/platform/tm/first/findContractInfoSignStateWeb"
        self.LOGGER.info("test_api_78dk_platform_tm_first_findContractInfoSignStateWeb请求地址:【{}】".format(requesturl))
        params = json.dumps({"uid": uid, })
        self.LOGGER.info("test_api_78dk_platform_tm_first_findContractInfoSignStateWeb请求参数：【{}】".format(params))
        response = requests.request(
                "POST", requesturl, headers=API_TEST_HEADERS, data=params
        )
        self.LOGGER.info("请求结果参数：【{}】".format(response.text))
    ...
    def test_api_78dk_platform_tm_after_viewAuditMonitors(self, enddate, pagecurrent, pagesize, qifascore, searchwhere, startdate,):
        """
        贷后列表
        依赖函数：test_{'msg': '返回消息文本', 'code': '返回码', 'data': "{'totalPage': '总页数', 'pageSize': '单页记录数', 'totalCount': '总记录数', 'currentPage': '当前页码', 'dataList': '列表', 'otherData': '其它数据'}"}
        依赖参数：
        ...
        params = json.dumps({"endDate": enddate, "pageCurrent": pagecurrent, "pageSize": pagesize, "qifaScore": qifascore, "searchWhere": searchwhere, "startDate": startdate, })
    ...
    def test_api_78dk_platform_tm_telephone_deleteTelephoneCheckInfo(self, uid,):
        """
        删除电核资料
        依赖函数：test_{'msg': '返回信息', 'code': '返回编号', 'data': '返回数据'}
        依赖参数：
        :return: response.text
        """
        requesturl = baseUrl + "/api/78dk/platform/tm/telephone/deleteTelephoneCheckInfo"
        self.LOGGER.info("test_api_78dk_platform_tm_telephone_deleteTelephoneCheckInfo请求地址:【{}】".format(requesturl))
        params = json.dumps({"uid": uid, })
        self.LOGGER.info("test_api_78dk_platform_tm_telephone_deleteTelephoneCheckInfo请求参数：【{}】".format(params))
        response = requests.request(
                "POST", requesturl, headers=API_TEST_HEADERS, data=params
        )
        self.LOGGER.info("请求结果参数：【{}】".format(response.text))

...
def select_infos(table_names, col_name, condition_column, condition_value_list):
    if isinstance(table_names, str):
        table_names = [table_names]
    for table_name in table_names:
        if condition_value_list is None:
...
def del_collection(platform_user_profile_uuid):
    ...
    根据系统用户uuid，删除催收人员
    """
    mydb.delete(la.DB,'Tbl_PlatformUserProfileCollection','platform_user_profile_uuid="{}"'.format(platform_user_profile_uuid))
...
if __name__ == '__main__':
    update_info('Tbl_MerchantProfile', 'audit_state="pending_review"',
                'merchant_uuid="d7620f7f4153471c8438fd6d857584c2"')

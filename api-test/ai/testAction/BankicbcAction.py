...
"""
@Author     : QA 
@File       : BankicbcAction.py
@desc       : 项目：ai 模块：BankICBC 接口方法封装
"""
...
baseUrl = MysqlConfig.get('BankICBC_apiURL', 'ai')
...
def test_api_bankabc_login(name, password, reqid, token):
    ...
    MysqlClent.update(MysqlClent.get_conn(), 't_rap', 'is_exe = "Y"', 'id = 1198')
...
def test_api_bankabc_get_sms(reqid, token):
    ...
    """
    start_time = time.time()
    MysqlClent.update(MysqlClent.get_conn(), 't_rap', 'is_exe = "Y"', 'id = 1199')
...
def test_api_bankabc_verify_sms(code, reqid, token):
    ...
    """
    start_time = time.time()
    MysqlClent.update(MysqlClent.get_conn(), 't_rap', 'is_exe = "Y"', 'id = 1200')
...
def test_api_bankabc_get_status(reqid, token):
    ...
    """
    start_time = time.time()
    MysqlClent.update(MysqlClent.get_conn(), 't_rap', 'is_exe = "Y"', 'id = 1201')
...
def test_api_bankabc_qrlogin(reqid, token):
    ...
    """
    start_time = time.time()
    MysqlClent.update(MysqlClent.get_conn(), 't_rap', 'is_exe = "Y"', 'id = 1202')
...
def test_api_bankabc_get_result(reqid, token):
    ...
    """
    start_time = time.time()
    MysqlClent.update(MysqlClent.get_conn(), 't_rap', 'is_exe = "Y"', 'id = 1203')
